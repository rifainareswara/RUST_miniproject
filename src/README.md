...
1.  **Buat Proyek Baru dengan Cargo**
    ...
    Perintah ini akan membuat direktori baru bernama `todo_cli` dengan struktur proyek standar Rust.
...
3.  **Kompilasi dan Jalankan Proyek**
    Di dalam direktori `todo_cli`, jalankan perintah:
...
## dissected Penjelasan Arsitektur Kode
...
1.  **Persistensi Data**: Menyimpan daftar tugas ke dalam sebuah file (misalnya format JSON atau CSV) sehingga data tidak hilang saat program ditutup, dan memuatnya kembali saat program dimulai.
...
3.  **Error Handling yang Lebih Baik**: Mengganti `.unwrap()` dengan penanganan `Result` yang lebih eksplisit (misalnya dengan `match` atau `if let Err(...)`) untuk membuat program lebih tangguh.
4.  **Fitur Tambahan**: Menambahkan fungsionalitas seperti menandai tugas sebagai selesai, memberi prioritas, atau menetapkan tenggat waktu.
